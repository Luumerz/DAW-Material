addEventListener("DOMContentLoaded", mostrarCategorias)
...
function mostrarCategorias() {
	...
	for (let categoria of CATEGORIAS) {
		texto += `<div class="col"><h1><span class="badge bg-info cursor-pointer" onclick="mostrarProductos(${categoria.id})">${categoria.nombre}</span></h1></div>`;
	}
	contenedorCategorias.innerHTML = texto;
}
...
function mostrarProductos(categoriaId) {
	...
	for (let producto of PRODUCTOS) {
		if (producto.categoria === categoriaId) {
			texto += `<div class="card col m-2" style="width: 18rem;">
	...
	contenedorProductos.innerHTML = texto;
}

function mostrarCaracteristicas(productoId) {
	let contenedorCaracteristicas = document.getElementById("caracteristicas");
	let texto = "";
	for (let producto of PRODUCTOS) {
		if (producto.id === productoId) {
			for (let caracteristica of producto.caracteristicas) {
				texto += `<li class="list-group-item d-flex justify-content-between lh-sm">
            				<h6 class="my-0">${caracteristica}</h6>
            			</li>`;
			}
		}
	}
	contenedorCaracteristicas.innerHTML = texto;
}